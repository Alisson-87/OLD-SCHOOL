...
import {
    ...
    Edit3, Trash2, CheckCircle, Info, AlertTriangle,
    Search, Shield, X, Plus, Minus, FileText, UserX,
    ArrowDownCircle, TrendingDown, TrendingUp, Clock, XCircle, Tag
} from 'lucide-react';
...
const sanitizeInput = v => typeof v === 'string' ? v.replace(/[<>]/g,'') : v;
...
function Modal({ title, onClose, children, wide }) {
    return (
        <div style={{position:'fixed',inset:0,backgroundColor:'rgba(0,0,0,0.6)',zIndex:1000,
            ...
            <div style={{backgroundColor:C.card,borderRadius:12,width:'100%',maxWidth:wide?740:500,
    ...
    return (
        <Modal title="Política de Privacidade — LGPD" onClose={onFechar} wide>
            <div style={{fontSize:13,lineHeight:1.7,color:C.text}}>
                ...
                <p style={{margin:'0 0 20px',color:C.muted}}>Os dados são armazenados localmente com criptografia AES-256. Nenhum dado é compartilhado com terceiros.</p>
                ...
                <div style={{backgroundColor:C.card,borderRadius:10,padding:'20px 22px',boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}>
                    ...
                    {stats.vendas_recentes.map((v,i)=>(
                        <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:`1px solid ${C.border}`,fontSize:13}}>
                            ...
                            <span style={{fontWeight:700,color:C.success}}>R$ {fmt.brl(v.TOTAL)}</span>
                        </div>
                    ))}
                </div>
            )}
        ...
        <div>
            <Field label="Produto" required error={erros.produto_id}>
                <select style={{...iSt(erros.produto_id),backgroundColor:'#fafafa'}} value={f.produto_id} onChange={e=>v('produto_id',e.target.value)}>
                    ...
                    {prods.map(p=><option key={p.ID} value={p.ID}>{p.NOME_PRODUTO} (Estoque atual: {p.QUANTIDADE})</option>)}
                </select>
            </Field>
            ...
            <Field label="Data de Vencimento do Pagamento">
            ...
            {total>0&&(
                <div style={{backgroundColor:'#fef2f2',border:`1px solid #fecaca`,borderRadius:8,padding:'10px 14px',marginBottom:14}}>
                    <p style={{margin:0,fontSize:13,color:C.danger,fontWeight:700}}>
                        Total da entrada: R$ {fmt.brl(total)} · Débito será gerado para o fornecedor selecionado
                    </p>
                </div>
            )}
            <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
...
function FornecedorForm({ item, onSalvo, onCancelar }) {
    ...
    const salvar = async () => {
        ...
        setLoading(true);
        try {
            const payload={nome:sanitizeInput(f.nome),cnpj:f.cnpj,endereco:sanitizeInput(f.endereco),telefone:f.telefone,email:f.email,contato_principal:sanitizeInput(f.contato_principal),devido:parseFloat(f.devido)||0,condicao:sanitizeInput(f.condicao)};
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 16px'}}>
            ...
            <div style={{gridColumn:'1/-1'}}>
                <Field label="Endereço">
                    <input style={iSt()} value={f.endereco} onChange={e=>v('endereco',e.target.value)} placeholder="Insira o endereço completo"/>
                </Field>
            </div>
        ...
        <div>
            ...
            <Tabela
                cols={[
                    ...
                    {key:'CONDICAO_PAGAMENTO',label:'Condição Pgto'},
                    {key:'VALOR_DEVIDO',label:'Dívida',render:v=><span style={{color:v>0?C.danger:C.success,fontWeight:700}}>R$ {fmt.brl(v)}</span>},
                ]}
                rows={filtered}
...
function ProdutoForm({ item, forns, onSalvo, onCancelar }) {
    const [f, setF] = useState({
        nome:'',codigo_barras:'',descricao:'',quantidade:'',preco:'',categoria:'',data_validade:'',fornecedor_id:'',
        ...(item?{nome:item.NOME_PRODUTO,codigo_barras:item.CODIGO_BARRAS||'',descricao:item.DESCRICAO||'',quantidade:item.QUANTIDADE,preco:item.PRECO,categoria:item.CATEGORIA||'',data_validade:item.DATA_VALIDADE||'',fornecedor_id:item.FORNECEDOR_ID||''}:{})
    });
    const [erros, setErros] = useState({});
    const [loading, setLoading] = useState(false);
    const v = (k,val) => setF(p=>({...p,[k]:val}));
    const validar = () => {
        ...
        if (f.preco!==''&&parseFloat(f.preco)<0) e.preco='Preço não pode ser negativo';
        if (f.quantidade!==''&&parseInt(f.quantidade)<0) e.quantidade='Quantidade não pode ser negativa';
        setErros(e); return !Object.keys(e).length;
    };
    const salvar = async () => {
        if (!validar()) return;
        setLoading(true);
        try {
            const payload={nome:sanitizeInput(f.nome),codigo_barras:f.codigo_barras||undefined,descricao:sanitizeInput(f.descricao),quantidade:parseInt(f.quantidade)||0,preco:parseFloat(f.preco)||0,categoria:f.categoria,data_validade:f.data_validade||undefined,fornecedor_id:f.fornecedor_id||undefined};
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 16px'}}>
            <div style={{gridColumn:'1/-1'}}>
                <Field label="Nome do Produto" required error={erros.nome}>
                    <input style={iSt(erros.nome)} value={f.nome} onChange={e=>v('nome',e.target.value.toUpperCase())} placeholder="Insira o nome do produto"/>
                </Field>
            </div>
            <Field label="Código de Barras" error={erros.codigo_barras}>
                <input style={iSt()} value={f.codigo_barras} onChange={e=>v('codigo_barras',masks.barras(e.target.value))} placeholder="Insira o código de barras"/>
            </Field>
            <Field label="Categoria" required error={erros.categoria}>
                <select style={{...iSt(erros.categoria),backgroundColor:'#fafafa'}} value={f.categoria} onChange={e=>v('categoria',e.target.value)}>
            ...
            <div style={{gridColumn:'1/-1'}}>
                <Field label="Descrição" required error={erros.descricao}>
                    <textarea style={{...iSt(erros.descricao),minHeight:70,resize:'vertical'}} value={f.descricao} onChange={e=>v('descricao',sanitizeInput(e.target.value))} placeholder="Descreva brevemente o produto"/>
                </Field>
            </div>
            <Field label="Quantidade em Estoque" error={erros.quantidade}>
                <input style={iSt(erros.quantidade)} type="number" min="0" value={f.quantidade} onChange={e=>v('quantidade',e.target.value)} placeholder="0"/>
            </Field>
            <Field label="Preço de Venda (R$)" error={erros.preco}>
                <input style={iSt(erros.preco)} type="number" step="0.01" min="0" value={f.preco} onChange={e=>v('preco',e.target.value)} placeholder="0,00"/>
            </Field>
            <Field label="Data de Validade">
                <input style={iSt()} type="date" value={f.data_validade} onChange={e=>v('data_validade',e.target.value)}/>
            </Field>
            <Field label="Fornecedor Principal">
                <select style={{...iSt(),backgroundColor:'#fafafa'}} value={f.fornecedor_id} onChange={e=>v('fornecedor_id',e.target.value)}>
                    <option value="">Selecione um fornecedor</option>
                    {forns.map(fn=><option key={fn.ID} value={fn.ID}>{fn.NOME_EMPRESA}</option>)}
                </select>
            </Field>
        ...
        <div>
            ...
            <Tabela
                cols={[
                    ...
                    {key:'PRECO',label:'Preço',render:v=>`R$ ${fmt.brl(v)}`},
            ...
            {modalEntrada&&(
                <Modal title="Registrar Entrada de Estoque" onClose={()=>setModalEntrada(false)}>
                    <EntradaModal prods={rows} forns={forns} onSalvo={()=>{setModalEntrada(false);load();}} onFechar={()=>setModalEntrada(false)}/>
                </Modal>
            )}
        </div>
    );
...
function Clientes() {
    ...
    const tentarSalvar = () => {
        if (!validar()) return;
        if (!editItem) { setPendente(f); setLgpdModal(true); }
        else salvar(f, true);
    };
    const salvar = async (dados, skipLgpd=false) => {
        try {
            const payload={nome:sanitizeInput(dados.nome),cpf:dados.cpf,tel:dados.tel,email:dados.email,endereco:sanitizeInput(dados.endereco),consentimento_lgpd:skipLgpd?undefined:true};
            ...
            setModal(false); setLgpdModal(false); setPendente(null);
            setF({nome:'',cpf:'',tel:'',email:'',endereco:''}); load();
    ...
    const del = async (id) => {
        ...
        try { await axios.delete(`${API}/clientes/${id}`); toast.success('Cliente removido!'); load(); } catch { toast.error('Erro'); }
    };
        ...
        <div>
            ...
            <Tabela
                cols={[
                    ...
                    {key:'TELEFONE',label:'Telefone'},
                    {key:'EMAIL',label:'E-mail'},
                    {key:'ULTIMA_COMPRA_DATA',label:'Última Compra',render:v=>fmt.data(v)},
                    {key:'ULTIMA_COMPRA_VALOR',label:'Valor',render:v=>v>0?`R$ ${fmt.brl(v)}`:'—'},
                ]}
                rows={rows}
                onEdit={r=>{setEditItem(r);setF({nome:r.NOME_COMPLETO,tel:r.TELEFONE||'',email:r.EMAIL||'',endereco:r.ENDERECO||''});setModal(true);}}
                onDelete={del}
                extraActions={r=><UserX size={15} title="Direito ao Esquecimento (LGPD Art. 18)" onClick={()=>lgpdEsquecer(r.ID)} style={{cursor:'pointer',color:C.warn,marginRight:12}}/>}
            />
            {modal&&(
                <Modal title={editItem?'Editar Cliente':'Novo Cliente'} onClose={()=>setModal(false)}>
                    <Field label="Nome Completo" required error={erros.nome}>
                        <input style={iSt(erros.nome)} value={f.nome} onChange={e=>vf('nome',e.target.value.toUpperCase())} placeholder="Nome completo do cliente"/>
                    </Field>
                    <Field label="CPF (opcional — armazenado com criptografia AES)">
                        <input style={iSt()} value={f.cpf} onChange={e=>vf('cpf',masks.cpf(e.target.value))} placeholder="000.000.000-00"/>
                    </Field>
                    <Field label="Telefone" required error={erros.tel}>
                        <input style={iSt(erros.tel)} value={f.tel} onChange={e=>vf('tel',masks.tel(e.target.value))} placeholder="(00) 00000-0000"/>
                    </Field>
                    <Field label="E-mail" error={erros.email}>
                        <input style={iSt(erros.email)} type="email" value={f.email} onChange={e=>vf('email',e.target.value)} placeholder="email@cliente.com"/>
                    </Field>
                    <Field label="Endereço">
                        <input style={iSt()} value={f.endereco} onChange={e=>vf('endereco',e.target.value)} placeholder="Endereço completo"/>
                    </Field>
    ...
    return (
        <div>
            ...
            <div style={{backgroundColor:C.card,borderRadius:10,padding:24,boxShadow:'0 2px 12px rgba(0,0,0,0.06)',marginBottom:16}}>
                ...
                {detalhe&&(
                    <div style={{backgroundColor:'#f8f8f8',borderRadius:8,padding:'12px 14px',marginTop:4,fontSize:13}}>
                        <p style={{margin:'0 0 4px',fontWeight:700}}>{detalhe.NOME_PRODUTO}</p>
                        {detalhe.CODIGO_BARRAS&&<p style={{margin:'0 0 2px',color:C.muted}}>Cód. Barras: {detalhe.CODIGO_BARRAS}</p>}
                    ...
                    <div style={{display:'flex',gap:10,flexWrap:'wrap',alignItems:'flex-end'}}>
                        <div style={{flex:1,minWidth:200}}>
                            <Field label="Fornecedor">
                                <select style={{...iSt(),backgroundColor:'#fafafa'}} value={fornSel} onChange={e=>setFornSel(e.target.value)}>
                                    <option value="">Selecione um fornecedor</option>
                                    {forns.map(fn=><option key={fn.ID} value={fn.ID}>{fn.NOME_EMPRESA}</option>)}
                                </select>
                            </Field>
                        ...
                        <div style={{paddingBottom:14}}>
                            <Btn onClick={associar}><Link2 size={15}/>Associar Fornecedor</Btn>
                        </div>
                    </div>
                </div>
            )}
            {detalhe?.fornecedores_associados?.length>0&&(
                <div style={{backgroundColor:C.card,borderRadius:10,padding:24,boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}>
                    <h3 style={{margin:'0 0 14px',fontSize:12,fontWeight:800,color:C.muted,textTransform:'uppercase',letterSpacing:.5}}>Fornecedores Associados a este Produto</h3>
                    <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                        ...
                        <tbody>{detalhe.fornecedores_associados.map(fn=>(
                            <tr key={fn.ID} style={{borderBottom:`1px solid ${C.border}`}}>
                                <td style={{padding:'10px 12px'}}>{fn.NOME_EMPRESA}</td>
                                <td style={{padding:'10px 12px',color:C.muted}}>{fmtCnpj(fn.CNPJ)}</td>
                                <td style={{padding:'10px 12px'}}><Btn variant="danger" small onClick={()=>desassociar(fn.ID)}>Desassociar</Btn></td>
                            </tr>
                        ))}</tbody>
                    </table>
                </div>
...
function PDV({ prods, clis }) {
    ...
    const gerarPDF = () => {
        ...
        const fatorDesc = temDesconto ? (1-TAXA_DESCONTO) : 1;
        autoTable(doc,{
            startY:76,
            head:[['ITEM / DESCRIÇÃO','QTD','UNIT (R$)',temDesconto?'UNIT C/ DESC.':null,'TOTAL (R$)'].filter(Boolean)],
            body:validos.map(it=>{
                const p=getProd(it.prod_id);
                const precoFinal=p?.PRECO*fatorDesc||0;
                const row=[p?.NOME_PRODUTO||'—',it.qtd,fmt.brl(p?.PRECO||0)];
                if (temDesconto) row.push(fmt.brl(precoFinal));
                row.push(fmt.brl(precoFinal*it.qtd));
                return row;
            }),
        ...
        if (temDesconto) {
            ...
            doc.text(`Desconto ${TAXA_DESCONTO*100}% (${formaPag}): - R$ ${fmt.brl(desconto)}`,14,y+6);
        }
        ...
        doc.text('Orçamento válido por 15 dias. Preços sujeitos a alteração sem aviso prévio.',14,y+(temDesconto?30:22));
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 340px',gap:20,alignItems:'start'}}>
                <div style={{backgroundColor:C.card,borderRadius:10,padding:24,boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:4}}>
                        <Field label="Cliente (opcional)">
                            <select style={{...iSt(),backgroundColor:'#fafafa'}} value={cliId} onChange={e=>setCli(e.target.value)}>
                                <option value="">Consumidor Final</option>
                                {clis.map(c=><option key={c.ID} value={c.ID}>{c.NOME_COMPLETO}</option>)}
                            </select>
                        </Field>
                        <Field label="Forma de Pagamento">
                            <select style={{...iSt(),backgroundColor:'#fafafa'}} value={formaPag} onChange={e=>setFormaPag(e.target.value)}>
                                {FORMAS_PAG.map(f=><option key={f} value={f}>{f}{FORMAS_DESCONTO.includes(f)?' — 10% OFF':''}</option>)}
                            </select>
                        </Field>
                    </div>
                    {temDesconto&&(
                        <div style={{display:'flex',alignItems:'center',gap:8,backgroundColor:'#f0fdf4',border:`1px solid #bbf7d0`,borderRadius:8,padding:'8px 14px',marginBottom:14}}>
                            <Tag size={14} style={{color:C.success}}/>
                            <span style={{fontSize:13,color:C.success,fontWeight:700}}>Desconto de 10% aplicado automaticamente para {formaPag}</span>
                        </div>
                    )}
                        ...
                        {!pago&&(
                            <div style={{flex:1,minWidth:160}}>
                                <Field label="Data de Vencimento">
                                    <input style={iSt()} type="date" value={dataVenc} onChange={e=>setDataVenc(e.target.value)}/>
                                </Field>
                            </div>
                        )}
                    </div>
                    ...
                    {itens.map((it,i)=>{
                        const p=getProd(it.prod_id);
                        const precoFinal=p?(p.PRECO*(temDesconto?1-TAXA_DESCONTO:1)):0;
                        return (
                            <div key={i} style={{display:'flex',gap:8,alignItems:'center',marginBottom:10}}>
                                ...
                                {p&&<span style={{fontSize:12,fontWeight:700,color:C.success,whiteSpace:'nowrap'}}>R$ {fmt.brl(precoFinal*it.qtd)}</span>}
                ...
                <div style={{backgroundColor:C.dark,borderRadius:10,padding:24,color:'#fff',position:'sticky',top:20}}>
                    ...
                    {itens.filter(it=>it.prod_id).map((it,i)=>{
                        const p=getProd(it.prod_id);
                        return p?<div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:8,fontSize:13}}><span style={{color:'#ccc'}}>{p.NOME_PRODUTO} x{it.qtd}</span><span style={{fontWeight:700}}>R$ {fmt.brl(p.PRECO*it.qtd)}</span></div>:null;
                    })}
                    {temDesconto&&(
                        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8,fontSize:12}}>
                            <span style={{color:'#4ade80'}}>Desconto 10% ({formaPag})</span>
                            <span style={{color:'#4ade80',fontWeight:700}}>- R$ {fmt.brl(desconto)}</span>
                        </div>
                    )}
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:10,marginTop:20}}>
                        <Btn variant="dark"onClick={registrar} disabled={loading}><CheckCircle size={15}/>{loading?'Registrando...':'Registrar Venda'}</Btn>
...
export default function App() {
    ...
    const NavBtn = ({ id, Icon, label }) => (
        <button onClick={()=>setAba(id)} style={{
            ...
            color:aba===id?'#fff':C.muted,
            fontWeight:700,fontSize:12,textTransform:'uppercase',letterSpacing:.5,transition:'all .2s',
        }}>
            <Icon size={16}/>{label}
        </button>
    );
        ...
        <div style={{display:'flex',minHeight:'100vh',backgroundColor:C.bg,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
            <style>{`
                *{box-sizing:border-box;}
                input:focus,select:focus,textarea:focus{border-color:${C.accent}!important;outline:none;box-shadow:0 0 0 3px rgba(255,77,0,0.1);}
            ...
            <aside style={{width:230,backgroundColor:C.card,padding:'22px 14px',borderRight:`1px solid ${C.border}`,display:'flex',flexDirection:'column',position:'sticky',top:0,height:'100vh',overflowY:'auto'}}>
                ...
                <div style={{marginTop:'auto',paddingTop:18,borderTop:`1px solid ${C.border}`,fontSize:10,color:C.muted,textAlign:'center',lineHeight:1.7}}>
                    <Shield size={12} style={{marginBottom:3}}/><br/>
